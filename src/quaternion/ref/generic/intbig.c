...
void ibz_complex_pow_mod(ibz_t *u, ibz_t *v, const ibz_t *u0, const ibz_t *v0, const ibz_t *exp, const ibz_t *N)
{
    ...
    ibz_t Ru, Rv, Bu, Bv, E, Tu, Tv, t1, t2;
    ibz_init(&Ru); ibz_init(&Rv);
    ibz_init(&Bu); ibz_init(&Bv);
    ibz_init(&E);  
    ibz_init(&Tu); ibz_init(&Tv); 
    ibz_init(&t1); ibz_init(&t2);

    // 1. Initialize R = Ru + i*Rv = 1 + 0i
    ibz_set(&Ru, 1);
    ibz_set(&Rv, 0);

    // 2. Initialize B = Bu + i*Bv = u0 + i*v0 mod N
    ibz_copy(&Bu, u0);
    ibz_copy(&Bv, v0);
    ibz_mod(&Bu, &Bu, N);
    ibz_mod(&Bv, &Bv, N);
    ...
    // 4. Binary fast exponentiation from right to left
    while (!ibz_is_zero(&E)) {
        // If the lowest point of the current index is 1
        if (ibz_is_odd(&E)) {
            // R = R * B mod N
            // Tu = Ru*Bu - Rv*Bv
            ibz_mul(&t1, &Ru, &Bu);
   ...
   // Release temporary variables
    ibz_finalize(&Ru); ibz_finalize(&Rv);
    ibz_finalize(&Bu); ibz_finalize(&Bv);
    ibz_finalize(&E);  
    ibz_finalize(&Tu); ibz_finalize(&Tv); 
