...
/**
 * @brief Constant-time random left O0-ideal of given prime norm N = 1 mod 4
 *
 * Much faster than previous quat_sampling_random_ideal_O0_given_norm()
 *
 * @param lideal Output: left O0-ideal of norm N = 1 mod 4
 * @param params Represent Integer parameters from the level-dependent constants
 * @param norm Prime norm of the ideal, denoted as N
 * @param j j^2 = -1 mod N, because Legendre Symbol (-1/N) = 1 when N = 1 mod 4
 * @param x0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @param y0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @returns 1 if success, 0 if no left O0-ideal found or randomness failed
 */
...
/**
 * @brief Constant-time random left O0-ideal of given prime norm N = 3 mod 4
 *
 *
 * @param lideal Output: left O0-ideal of norm N = 3 mod 4
 * @param params Represent Integer parameters from the level-dependent constants
 * @param norm Prime norm of the ideal, denoted as N
 * @param u0 u0^2 + v0^2 == 1 mod N
 * @param v0 u0^2 + v0^2 == 1 mod N
 * @param x0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @param y0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @returns 1 if success, 0 if no left O0-ideal found or randomness failed
 */
int quat_constant_random_O0_ideal_norm_3mod4(quat_left_ideal_t *lideal,
                                             const quat_represent_integer_params_t *params,
                                             const ibz_t *norm,
                                             const ibz_t *u0,
                                             const ibz_t *v0,
...
/**
 * @brief Constant-time random left O0-ideal of given prime norm N = 1 mod 4 or N = 3 mod 4
 *
 * Much faster than previous quat_sampling_random_ideal_O0_given_norm()
 *
 * @param lideal Output: left O0-ideal of norm N = 1 mod 4 or N = 3 mod 4
 * @param params Represent Integer parameters from the level-dependent constants
 * @param norm Prime norm of the ideal, denoted as N
 * @param x0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @param y0 x0^2 + y0^2 == c mod N, c = -p^(-1)
 * @param minus_y0 -y0 mod N, precomputed variable for optimizing runtime efficiency
 * @param x0_plus_y0 (x0 + y0) mod N, precomputed variable for optimizing runtime efficiency
 * @returns 1 if success, 0 if no ideal found or randomness failed
 */
...
/**
 * @brief Generate random left O0-ideal of given non-prime norm N with known factors (N is odd)
 *
 *
 * @param lideal Output: left O0-ideal of non-prime norm N = q_1^(e_1)q_2^(e_2)...q_n^(e_n)
 * @param params Represent Integer parameters from the level-dependent constants
 * @param norm Non-prime norm of the ideal, denoted as N
 * @param primes Prime factor array q_i
 * @param powers Prime factor power array q_i^(e_i)
 * @param num_factors Number of factors n
 * @returns 1 if success, 0 if no left O0-ideal found or randomness failed
 */
